...
class KvRepresentation(nn.Module):
    def __init__(self, hidden=128, in_head=[6, 4], out_head=2):
        super().__init__()
        self.linear = nn.Linear(hidden*in_head[0]*in_head[1], hidden*in_head[1])
        self.norm = Qwen2RMSNorm(hidden*in_head[1])
    ...
    def forward(self, x, skip_x):
        ...
        # y = x.clone()
        x = self.linear(x) # -> [batch_size, seq_len, head2*head_dim]
        x = self.norm(x)
        # x = self.activate_1(x)
        x = x + skip_x
        
        x = self.compress_to_tgtdim_v2(x) # -> (batch_size, seq_len, head2*head_dim)
...
class AdaptiveMask(nn.Module):
    def __init__(self):
        super().__init__()
        self.k_mask = nn.Parameter(torch.ones(128*8*4))
        self.v_mask = nn.Parameter(torch.ones(128*8*4))
    
    def forward(self, key, value):
        return key * self.k_mask, value * self.v_mask
...
class PaliGemmaWithExpertModel(PreTrainedModel):
    ...
    def __init__(self, config: PaliGemmaWithExpertConfig, init_load = False, init_path = None):
        ...
        self.kv_mask = nn.ModuleList(AdaptiveMask() for _ in range(self.num_layers))
